...
from metrics_toolbox.metrics.registry import METRIC_REGISTRY
...
def test_evaluator_builder_add_metric_default():
    ...
    builder = EvaluatorBuilder().add_metric(MetricNameEnum.ROC_AUC_BINARY)

    spec = builder._metric_specs[0]
    assert spec.metric_cls.name == MetricNameEnum.ROC_AUC_BINARY
    assert spec.reducers == (
        MetricReducerEnum.LATEST,
...
def test_evaluator_builder_add_metric_with_params():
    ...
    spec = builder._metric_specs[0]
    assert spec.metric_cls.name == MetricNameEnum.ROC_AUC_CLASS
    assert spec.reducers == (MetricReducerEnum.MEAN, MetricReducerEnum.MIN)
    assert spec.class_name == "A"

    spec = builder._metric_specs[1]
    assert spec.metric_cls.name == MetricNameEnum.ROC_AUC_BINARY
    assert spec.reducers == (
        MetricReducerEnum.MINMAX,
        MetricReducerEnum.LATEST,
...
def test_evaluator_builder_from_dict():
    ...
    builder = EvaluatorBuilder().from_dict(cfg).add_metric(MetricNameEnum.ROC_AUC_MICRO)
    print(builder)
    ...
    spec = builder._metric_specs[1]
    assert spec.metric_cls.name == MetricNameEnum.ROC_AUC_MACRO
    assert spec.reducers == (MetricReducerEnum.MIN, MetricReducerEnum.MINMAX)
    assert spec.class_name is None, "Default spec class_name should be None"
    ...
    assert spec.metric_cls.name == MetricNameEnum.ROC_AUC_MICRO
    assert spec.reducers == (
        MetricReducerEnum.LATEST,
    ), "Default spec reducer should be LATEST"
    assert spec.class_name is None, "Default spec class_name should be None"
...
def test_evaluator_builder_add_metric_with_unsuported_params():
    ...
    with pytest.raises(ValueError):
        builder.add_metric(
            MetricNameEnum.ROC_AUC_BINARY, reducers=("unsupported_reducer",)
        )

    with pytest.raises(ValueError):
        builder.add_metric(MetricNameEnum.ROC_AUC_BINARY, reducers=(123,))

    with pytest.raises(ValueError):
        builder.add_metric(MetricNameEnum.ROC_AUC_CLASS, reducers=(None,))

    # The add_metric takes **kwargs, and those are passed to MetricSpec.
    with pytest.raises(TypeError):
        builder.add_metric(MetricNameEnum.ROC_AUC_CLASS, non_existing_param=True)
